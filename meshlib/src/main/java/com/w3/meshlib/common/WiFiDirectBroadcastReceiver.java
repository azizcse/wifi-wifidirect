    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action)) {
            ...
            if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED) {
                MeshLog.v(  "WiFi P2P is active");
            } else {
                MeshLog.v( "WiFi P2P isn't active");
            }

        } else if (WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION.equals(action)) {

            MeshLog.v( "WIFI_P2P_PEERS_CHANGED_ACTION...");
                ...
                wiFiP2PInstance.getWifiP2pManager().requestPeers(wiFiP2PInstance.getChannel(), new WifiP2pManager.PeerListListener() {
                    ...
                    public void onPeersAvailable(WifiP2pDeviceList peers) {
                        if (!peers.getDeviceList().isEmpty()) {
                            MeshLog.v( "Peers detected:");

                            for (WifiP2pDevice device : peers.getDeviceList()) {
                                MeshLog.v(  "\tDevice Name: " + device.deviceName);
                                MeshLog.v(  "\tDevice Address: " + device.deviceAddress);
                            }
                        } else {
                            MeshLog.v( "No peers detected");
                        }
                    }
                });
            }
            ...
            MeshLog.v( "WIFI_P2P_THIS_DEVICE_CHANGED_ACTION : " + device.deviceName);
            MeshLog.v( "This device address: " + device.deviceAddress);

            if (wiFiP2PInstance.getThisDevice() == null) {
                wiFiP2PInstance.setThisDevice(new GroupDevice(device));
            }
        }
    }
