...
public class ChatService {
	...
	private synchronized void setState(int state) {
		...
		handler.obtainMessage(MainActivity2.MESSAGE_STATE_CHANGE, state, -1)
				.sendToTarget();
	}
	...
	public synchronized void connected(BluetoothSocket socket,
		...
		Message msg = handler.obtainMessage(MainActivity2.MESSAGE_DEVICE_NAME);
		Bundle bundle = new Bundle();
		bundle.putString(MainActivity2.DEVICE_NAME, device.getName());
		msg.setData(bundle);
	...
	private void connectionFailed() {
		Message msg = handler.obtainMessage(MainActivity2.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(MainActivity2.TOAST, "Unable to connect device");
		msg.setData(bundle);
		handler.sendMessage(msg);
	...
	private void connectionLost() {
		Message msg = handler.obtainMessage(MainActivity2.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(MainActivity2.TOAST, "Device connection was lost");
		msg.setData(bundle);
		handler.sendMessage(msg);
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					handler.obtainMessage(MainActivity2.MESSAGE_READ, bytes, -1,
							buffer).sendToTarget();
				} catch (IOException e) {
					connectionLost();
					// Start the service over to restart listening mode
		...
		public void write(byte[] buffer) {
			try {
				...
				handler.obtainMessage(MainActivity2.MESSAGE_WRITE, -1, -1,
						buffer).sendToTarget();
			} catch (IOException e) {
			}
		}
